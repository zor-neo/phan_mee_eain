...
| State-changing GET routes | GET should be safe and idempotent. | Deletes and role changes can be triggered accidentally or via shared links. | Move those actions to POST, PATCH, or DELETE routes with CSRF protection. | Converted admin promote/demote/delete, category delete, author content delete, save toggle, and mark-seen actions to POST/DELETE routes with CSRF-backed forms or AJAX. Added route regression tests. | Solved |
...
| RBAC view mode, report cooldown, and author content hardening | Session-scoped view modes can preserve admin access while preventing hidden role mutation. Report throttling should be enforced server-side. Content creation should survive validation failure and keep attachment metadata separate from the main post. | Admins could impersonate roles unsafely, reports could be spammed, and authors could lose work or bypass file validation. | Store `acting_view_mode` in session, block writes in read-only author mode, enforce a 24-hour report cooldown, add attachment metadata tables, and keep invalid form data in `old()`. | Added a read-only middleware, admin view-mode switch, report cooldown checks, content resource storage, optional featured images, responsive feed controls, and masked auth passwords. | Solved |
